...
#ifdef HAS_RNTUPLE_SUPPORT
//*****************************************************************//
void QwBeamLine::ConstructNTupleAndVector(QwTrackedRNTupleModel& model, TString& prefix, std::vector<Double_t>& values, std::vector<std::shared_ptr<Double_t>>& fieldPtrs)
{
  for(size_t i = 0; i < fClock.size(); i++)
