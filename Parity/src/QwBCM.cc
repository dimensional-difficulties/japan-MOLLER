...
#ifdef HAS_RNTUPLE_SUPPORT
template<typename T>
void QwBCM<T>::ConstructNTupleAndVector(QwTrackedRNTupleModel& model, TString& prefix, std::vector<Double_t>& values, std::vector<std::shared_ptr<Double_t>>& fieldPtrs)
{
  if (this->GetElementName()==""){
    //  This channel is not used, so skip
  } else
