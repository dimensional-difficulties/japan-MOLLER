...
#ifdef HAS_RNTUPLE_SUPPORT
void QwMollerDetector::ConstructNTupleAndVector(QwTrackedRNTupleModel& model, TString& prefix, std::vector<Double_t>& values, std::vector<std::shared_ptr<Double_t>>& fieldPtrs)
{
  for(size_t i = 0; i < fSTR7200_Channel.size(); i++){
    for(size_t j = 0; j < fSTR7200_Channel[i].size(); j++){
