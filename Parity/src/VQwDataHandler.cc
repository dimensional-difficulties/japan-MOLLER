...
#ifdef HAS_RNTUPLE_SUPPORT
void VQwDataHandler::ConstructNTupleAndVector(QwTrackedRNTupleModel& model, TString& prefix, std::vector<Double_t>& values, std::vector<std::shared_ptr<Double_t>>& fieldPtrs)
{
  for (size_t i = 0; i < fOutputVar.size(); ++i) {
